...
def inference(
    ...
    audio_feature_result = audio_processor.get_audio_feature(audio_path)
    if audio_feature_result is None:
        raise ValueError(f"Invalid audio path or audio cannot be loaded: {audio_path}")
    ...
    if len(input_latent_list) == 0:
        raise RuntimeError(
            "No valid face was detected in the input video. Please try another video or adjust bbox_shift."
        )
